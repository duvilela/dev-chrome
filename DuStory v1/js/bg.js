          ...
          return {
            wt: function () {
              return chrome.cookies
                .get({ url: "https://www.instagram.com", name: "csrftoken" })
                .then(function (t) {
                  if (t && t.value) return t.value;
                  return chrome.cookies.get({ url: "https://instagram.com", name: "csrftoken" }).then(function (e) {
            ...
            yt: function () {
              return chrome.cookies
                .get({ url: "https://www.instagram.com", name: "ig_did" })
                .then(function (t) {
                  if (t && t.value) return t.value;
                  return chrome.cookies.get({ url: "https://instagram.com", name: "ig_did" }).then(function (e) {
                    return e && e.value ? e.value : "";
                  });
                })
                .catch(function () {
  ...
  ((() => {
    ...
    (() => {
      ...
      const a = {
          ...
          Zt: function (t) {
            const e = this;
            if (!t || !t.reels_media || !t.reels_media.length || !t.reels_media[0]) {
              return { username: "", edges: [] };
            }
            const n =
          ...
          de: function (t) {
            ...
            return (
              ...
              o()
                ...
                .then(function (t) {
                  t["x-ig-app-id"] = t["x-ig-app-id"] || "936619743392459";
                  t["x-asbd-id"] = t["x-asbd-id"] || "129477";
                  return (
                                        ...
                                        : "get_user_saved_collection_posts" ===
                                          ...
                                          : "get_download_started_event" ===
                                            ...
                                            : "remove_started_event" === t.title
                                              ? (m().Lt(n), !0)
                                              : "get_active_video_url" === t.title
                                                ? (chrome.tabs.query({ active: !0, currentWindow: !0 }).then(function (tabs) {
                                                    const tabId = (e.tab && e.tab.id) || (tabs && tabs[0] && tabs[0].id);
                                                    if (!tabId) return n(null);
                                                    chrome.scripting.executeScript({
                                                      target: { tabId: tabId },
                                                      world: "MAIN",
                                                      func: () => {
                                                        function isUrl(val) {
                                                          return typeof val === "string" && val.indexOf("https://") === 0 &&
                                                            (val.indexOf(".mp4") !== -1 || val.indexOf("/o1/v/") !== -1 || val.indexOf("bytestream") !== -1 || val.indexOf("cdninstagram.com") !== -1 || val.indexOf("fbcdn.net") !== -1);
                                                        }
                                                        function searchFiber(obj, depth, visited) {
                                                          if (!obj || typeof obj !== "object" || depth > 8 || visited.has(obj)) return null;
                                                          visited.add(obj);
                                                          const keys = ["hdSrc", "hd_src", "playbackUrl", "progressiveUrl", "sdSrc", "sd_src", "video_url", "url", "src"];
                                                          for (let k of keys) {
                                                          ...
                                                          for (let k in obj) {
                                                            ...
                                                            if (val && typeof val === "object") {
                                                              const res = searchFiber(val, depth + 1, visited);
                                                              if (res) return res;
                                                            }
                                                          }
                                                          return null;
                                                        }
                                                        const videos = Array.prototype.filter.call(document.querySelectorAll("video"), v => v.offsetHeight > 0 || v.offsetWidth > 0);
                                                        for (let i = videos.length - 1; i >= 0; i--) {
                                                          ...
                                                          if (fiberKey) {
                                                            ...
                                                            let f = v[fiberKey];
                                                            for (let u = 0; f && u < 40; u++, f = f.return) {
                                                              const found = searchFiber(f.memoizedProps, 0, visited);
